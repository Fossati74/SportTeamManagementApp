...
export const CarpoolManager = () => {
  ...
  // Utilisation du hook global pour synchroniser les fréquences
  ...
  const generateMonthWeekends = () => {
    const saturdays = getSpecificDaysInMonth(monthOffset, 6); // 6 = Samedi
                  ...
                  return (
                    <tr key={item.id} className="text-sm hover:bg-slate-700/20 transition-colors group">
                      <td className="px-6 py-4 text-slate-300 font-medium whitespace-nowrap">
                        {formatDateFr(item.weekend_date!)}
                      </td>
...
// --- COMPOSANT CARTE WEEK-END REFACTORE ---
const WeekendCard = ({ weekendDate, existing, players, proposals, onRefresh, isAdmin, playerStats, onDelete, onAssign }: any) => {
  ...
  const renderNames = (ids: string[]) => {
    const assigned = ids.filter(Boolean).map((id) => players.find((p: Player) => p.id === id)).filter(Boolean);
    ...
    <div className={`p-5 rounded-2xl border transition-all ${existing ? "bg-slate-900/60 border-slate-700 shadow-lg" : "bg-slate-900/20 border-slate-800 border-dashed"}`}>
      ...
      {proposals.length > 0 && isAdmin && !isEditing && (
        <div className="mb-4 space-y-2">
          {proposals.map((p: Player) => (
            <div key={p.id} className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl flex items-center justify-between">
              <span className="text-amber-500 text-xs font-bold uppercase tracking-tight">{p.first_name} est volontaire</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[ {team: team1, setTeam: setTeam1, label: "Voiture 1", color: "green" }, {team: team2, setTeam: setTeam2, label: "Voiture 2", color: "blue" }].map((v, idx) => (
              ...
              <div className="space-y-2">
                {v.team.map((id, i) => (
                  <div key={i} className="flex gap-2">
                    <select value={id} onChange={(e) => { const n = [...v.team]; n[i] = e.target.value; v.setTeam(n); }} className={`flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white outline-none focus:ring-2 focus:ring-${v.color}-500`}>
                      <option value="">Choisir...</option>
                      {players.filter((p: Player) => p.carpooling).map((p: Player) => {
                        const isChosen = [...team1, ...team2].some((cid) => cid === p.id && id !== p.id);
                        return <option key={p.id} value={p.id} disabled={isChosen}>{p.first_name} {p.last_name} ({playerStats[p.id]?.carpoolCount || 0}x)</option>;
                      })}
                    </select>
                    {v.team.length > 1 && <button onClick={() => v.setTeam(v.team.filter((_, idx) => idx !== i))} className="p-2 text-slate-500 hover:text-red-400"><UserMinus size={16} /></button>}
                  </div>
                ))}
                {v.team.length < 5 && <button onClick={() => v.setTeam([...v.team, ""])} className={`text-[10px] text-${v.color}-400 flex items-center gap-1 font-bold uppercase py-1`}><Plus size={12} /> Ajouter</button>}
              </div>
            ) : renderNames(v.team)}
          </div>
        ))}
      ...
      {isEditing && (
        <div className="mt-6 pt-5 border-t border-slate-800 flex gap-3">
          <button onClick={handleSave} className="flex-1 bg-green-600 hover:bg-green-500 text-white py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all uppercase tracking-widest shadow-lg shadow-green-900/20"><Check size={18} /> Valider</button>
