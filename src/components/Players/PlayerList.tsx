...
export const PlayerList = () => {
  ...
  // Utilisation du Hook personnalisé pour récupérer toutes les stats calculées
  ...
  const handleModalClose = () => {
    ...
    fetchPlayers();
    refreshStats(); // On rafraîchit les stats via le hook
  };
  ...
  const filteredPlayers = players.filter((p) => {
    ...
    return (
      matchesLicense &&
      fuzzyMatch(`${p.first_name} ${p.last_name}`, searchQuery, true)
    );
  });
  ...
  const sortedPlayers = [...filteredPlayers].sort((a, b) => {
    const aTotal =
      ...
      (playerStats[a.id]?.carpoolCount || 0);
    const bTotal =
      (playerStats[b.id]?.aperoCount || 0) +
      (playerStats[b.id]?.carpoolCount || 0);
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-slate-400">
            ...
            <span className="text-green-400 font-semibold">
              Amendes: {formatPrice(totalFinesGlobal)} €
            </span>
          </div>
        </div>
        {isAdmin && (
          <button
            ...
            className="w-full sm:w-auto bg-green-600 hover:bg-green-500 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95"
          >
      ...
      {sortedPlayers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedPlayers.map((player) => (
            <div
              key={player.id}
              className="bg-slate-800 rounded-2xl p-5 border border-slate-700 hover:border-green-500/40 transition-all group"
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3 overflow-hidden">
                  {player.photo_url ? (
                    ...
                    <div className="w-12 h-12 rounded-full bg-slate-700 flex items-center justify-center shrink-0 text-slate-400">
                      <User size={24} />
                    </div>
                  )}
                  <div className="truncate">
                    ...
                    <div className="flex gap-3 mt-1 text-slate-400">
                      {player.thursday_aperitif && (
                        <div className="flex items-center gap-1">
                          <Wine size={14} className="text-orange-400" />
                          <span className="text-xs">
                            {playerStats[player.id]?.aperoCount || 0}
                          </span>
                        </div>
                      )}
                      {player.scoreboard && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs">
                            {playerStats[player.id]?.matchCount || 0}
                          </span>
                        </div>
                      )}
                      {player.carpooling && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs">
                            {playerStats[player.id]?.carpoolCount || 0}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {((playerStats[player.id]?.finesTotal || 0) > 0 ||
                  (player.paid_amount || 0) > 0) && (
